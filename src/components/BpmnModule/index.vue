<template>
    ...
    <div class="bpmn-module">
        <div class='bpmn-canvas' ref="canvas"></div>
        <div class="bpmn-properties-panel">
            <CustomPropertiesPanel :modeler="modeler" :customData="customData" @setCustomData="setCustomData" />
        </div>
    </div>
</template>
<script setup>
import { onMounted, ref, watchEffect ,toRaw} from "vue"
import CustomModeler from './CustomModeler/index';
import customTranslate from './CustomTranslate/customTranslate';
import CustomPropertiesPanel from './CustomPropertiesPanel/index.vue';
...
onMounted(() => {
    ...
    customModeler._importXML().then(function (result) {
...
const save = async () => {
    ...
    const customList = Object.entries(customData.value)
    const startBpmn = customList.find(([k, v]) => {
        return v.type == "bpmn:StartEvent"
    })
    const processBpmn = customList.find(([key, value], index) => {
        // 修复不能重复保存bug
        let children = customData.value

        Object.entries(customData.value).forEach(([k, v], index) => {
    ...
    await api.PostApi(`/bpmn/saveOrUpdate`, {
        bpmnId: processBpmn.id,
        file: new File([xml], `${processBpmn.id}.xml`, {
            type: 'application/xml'
        })
    })
    await api.PostApi(`/bpmn/saveOrUpdate`, {
        bpmnId: processBpmn.id,
        file: new File([JSON.stringify(toRaw(processBpmn))], `${processBpmn.id}.json`, {
            type: 'application/json'
        })
    })
    console.log(processBpmn)
}

const setCustomData = ({ id, ...data }, state) => {
    const item = { ...customData.value }
    if (state == "remove") delete item[id]
    else item[id] = { id, ...data }
    customData.value = item
    console.log("setCustomData", item)
}
</script>
<style scoped>
.bpmn-top {
...
/* css控制
在流程图测试的时候隐藏追加面板（djs-context-pad） */
.bjs-container.simulation {
    .djs-context-pad {
        display: none;
    }
